<template>
	<div class="list_container">
		<ul v-load-more="loaderMoreMethod" v-if="tenementListArr.length" type="1">
			<div v-for="item in tenementListArr" tag='li' :key="item.ID" class="house_li" @click="goToDetailPage(item.ID)">
...
import {getSameHouseList} from 'src/service/house'
...
export default {
	...
	methods: {
		async initData(){
			//获取数据
			let res = await getSameHouseList('123' ,this.offset);
